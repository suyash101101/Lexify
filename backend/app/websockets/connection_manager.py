...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, room_id: str, user_address: str):
        ...
        case = postgres_client.get_case(room_id)
        if not case:
            raise HTTPException(status_code=404, detail="Case not found")
        
        # Check if the user ID matches the lawyer1_address
    ...
    async def broadcast_to_room(self, message: dict, room_id: str):
        if room_id in self.active_rooms:
            ...
            postgres_client.append_chat_message(room_id, message)
    ...
    async def get_room_messages(self, room_id: str) -> List[dict]:
        ...
        return postgres_client.get_chat_messages(room_id)
