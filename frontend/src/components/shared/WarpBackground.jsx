...
const Beam = ({
  ...
  // Using emerald-themed colors with varying opacity for depth
  const colors = [
    'hsl(152, 76%, 60%)',  // Bright emerald
    'hsl(148, 64%, 50%)',  // Deep emerald
    'hsl(160, 84%, 45%)',  // Rich emerald
  ];
  const color = colors[Math.floor(Math.random() * colors.length)];
  // Taller aspect ratios to suggest courthouse columns
  const ar = Math.floor(Math.random() * 4) + 12; // 12-16 ratio for tall, stately columns
  ...
  return (
    <motion.div
      style={{
        ...
        "--width": `${width}`,
        "--aspect-ratio": `${ar}`,
        "--background": `linear-gradient(${color}, transparent 80%)`,
      }}
      className={`absolute left-[var(--x)] top-0 [aspect-ratio:1/var(--aspect-ratio)] [background:var(--background)] [width:var(--width)] opacity-[0.15] backdrop-blur-sm`}
...
export const WarpBackground = ({
  ...
  beamDelayMin = 0,
  beamDuration = 3,
  gridColor = "rgba(52, 211, 153, 0.08)", // Subtle emerald grid
  ...props
}) => {
